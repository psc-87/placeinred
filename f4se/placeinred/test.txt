...
static uintptr_t* ModAngleXFinder = nullptr;
static uintptr_t* ModAngleYFinder = nullptr;
...
static bool ModCurrentRefAngle(char axis, float fModify)
{
	...
	if (ModAngleXFinder && ModAngleYFinder && ModAngleZFinder) {
		...
		if (ref) {
			if (axis == 'x' || axis == 'X') {
				ModAngleX(ref, fModify);
				return true;
			}

			if (axis == 'y' || axis == 'Y') {
				ModAngleY(ref, fModify);
				return true;
			}

			if (axis == 'z' || axis == 'Z') {
				ModAngleZ(ref, fModify);
				return true;
			}
		}
	}
...
if (ModAngleXFinder && ModAngleYFinder && ModAngleZFinder) {
	...
	ModAngleZRel32 = uintptr_t(ModAngleZFinder) - RelocationManager::s_baseAddr;
	RelocAddr <_ModAngleXYZ> GimmeModAngleX(ModAngleXRel32);
	RelocAddr <_ModAngleXYZ> GimmeModAngleY(ModAngleYRel32);
	RelocAddr <_ModAngleXYZ> GimmeModAngleZ(ModAngleZRel32);
	ModAngleX = GimmeModAngleX;
...
bool TransmitConnectedPowerLatent(UInt32 stackId, TESObjectREFR* refr)
{
	...
	for (UInt32 i = 0; i < extraData->points.count; i++)
	{
		...
		if (parent != root && refr->parentCell)
		{
			...
			if (world)
			{
				TESObjectREFR* connected = GetObjectAtConnectPoint(refr, &worldPos, world, 8.0 f);
				if (connected)
				{
							...
							if (form)
							{
								...
								if (wireRef->baseForm->formID == splineFormId)
								{
									...
									if (wirePowerLinks && wirePowerLinks->connections.count == 2)
									{
										...
										{
											//Check if we are at the starting end.
											wirePowerLinks->connections.GetNthItem(1, formID);
	...
	for (int i = 0; i < objectLines.Length(); i++)
	{
		...
		while (startIndex >= 0)
		{
			...
			else if (elementIndex == 6)
			{
				...
				entry.Set<float>("angleX", (float)(((long double)ToFloat(elementString)) / 180.0 l * PI));
			}
			else if (elementIndex == 7)
			{
				AppendToLog("Found angleY: " + elementString);
				entry.Set<float>("angleY", (float)(((long double)ToFloat(elementString)) / 180.0 l * PI));
			}
			else if (elementIndex == 8)
			{
				AppendToLog("Found angleZ: " + elementString);
				entry.Set<float>("angleZ", (float)(((long double)ToFloat(elementString)) / 180.0 l * PI));
			}
...
PatternReferenceEntry GetPatternReferenceInformation(StaticFunctionTag* base, UInt32 slot)
{
	...
	for (int i = 0; i < keys.Length(); i++)
	{
		...
		else if (elementKey == "angle_x")
		{
			...
			entry.Set("angleX", (float)(((long double)ToFloat(elementValue.c_str())) / 180.0 l * PI));
		}
		else if (elementKey == "angle_y")
		{
			AppendToLog("Found angleY: " + elementValueString);
			entry.Set("angleY", (float)(((long double)ToFloat(elementValue.c_str())) / 180.0 l * PI));
		}
		else if (elementKey == "angle_z")
		{
			AppendToLog("Found angleZ: " + elementValueString);
			entry.Set("angleZ", (float)(((long double)ToFloat(elementValue.c_str())) / 180.0 l * PI));
		}
	}
...
BGSConstructibleObject* GetConstructibleObjectByCreatedObject_(StaticFunctionTag* base, UInt32 formId)
{
	...
	for (UInt32 i = 0; i < theDH->arrCOBJ.count; i++)
	{
		...
		if (conObj->createdObject->formType == 94)
		{
			//links to a formlist
